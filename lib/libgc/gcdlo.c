...
#ifndef PICOMEM_DISPLAYLINK_SCALE
#define PICOMEM_DISPLAYLINK_SCALE 2
#endif
long g_DLOScaleFactor;
static long DLO_ScaleFactor(void)
{
    return g_DLOScaleFactor;//return PICOMEM_DISPLAYLINK_SCALE < 1 ? 1 : PICOMEM_DISPLAYLINK_SCALE;
}
...
GCBOOL DLO_HWSetupDevice(GCDEVICE *pDevice, GCBITMAP *pDisplaySurface, void *param)
{
    ...
    long width = pMode->view.width;
    long height = pMode->view.height;
    height-=1;
    long scale = width > 1000 ? 4 : 2;
    g_DLOScaleFactor = scale;
    width /= scale;
    height /= scale;

    printf("gc: displaylink %ux%u scale %ld effective %ldx%ld\n",
           pMode->view.width, pMode->view.height, scale, width, height);
    GCInitializeBitmap(pDisplaySurface, width, height, (unsigned long)(uintptr_t)param, GCDeviceDisplayOnly);
    return GCTRUE;
}
...
void Task_UpdatePalette()
{
    if (gc.bitmap.handle) {
        ...
        if (mcga_register_dirty) {
            DrawMcgaRegisters(&gc, registerX, registerY, registerWidth, registerHeight);
            mcga_register_dirty = false;
        }
        if (post_code_dirty) {
            DrawPostCodes(&gc, postX, postY, paletteWidth);
            post_code_dirty = false;
        }
        DrawPalette(&gc, paletteX, paletteY, paletteWidth, paletteHeight);
        if (redrawPanelSeparators) {
            draw_overlay_separator(&gc, mainSeparatorX, 0, 1, GCHeight(&gc));
            draw_overlay_separator(&gc, mainSeparatorX, postSeparatorY, GCWidth(&gc) - mainSeparatorX, 1);
        }
...
void dlo_device_configured (dlo_dev_t uid)
{
    ...
    mcga_overlay_initialized = false;
    mcga_mark_all_register_rows_dirty();
}
